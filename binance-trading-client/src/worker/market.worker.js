...
let currentDataSource = 'backend';
// 🌟 这里记录所有存活的流
const activeSubscriptions = new Set();
const defaultStreams = ['!miniTicker@arr', '!markPrice@arr@1s'];
function broadcastToPorts(type, payload) {
...
async function connectPublicStream() {
    ...
    if (currentDataSource === 'backend') {
        signalRConnection = new signalR.HubConnectionBuilder()
            ...
            .build();
        signalRConnection.on("ReceiveMarketData", (rawJson) => {
            try {
                processMarketData(JSON.parse(rawJson));
            }
            catch (e) { }
        });
        // 🌟 新增：接收后端到币安的真实延迟并向所有窗口广播
        signalRConnection.on("ReceiveBackendLatency", (ms) => {
        ...
        publicWs.onmessage = (event) => {
            try {
                processMarketData(JSON.parse(event.data));
            }
            catch (e) { }
        };
...
function connectUserDataStream() {
    ...
    userDataWs.onmessage = (event) => {
        try {
            broadcastToPorts('ACCOUNT_DATA', JSON.parse(event.data));
        }
        catch (err) { }
    };
